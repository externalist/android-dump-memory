...
int main(int argc, char **argv) {
    ...
    if (argc == 5 || argc == 6) {
        ...
        long start_address;
        sscanf(argv[3], "0x%x", (unsigned int *)&start_address);
        int total_bytes;
...
void my_printf(bool enable, const char *format, ...){
    if(enable == true){
        va_list args;
...
dump_memory(int pid, unsigned int start_address, int total_bytes, char* option, char* search_string) {
	...
	for (address=start_address; address<start_address+total_bytes; address++) {
        ...
        if (show_flag){
            if (address%16 == 0){
                my_printf(show_flag, "0x%08x : ", address);
            }
                ...
                if (unicode_flag == false){
                    if (character == search_string[search_string_index]){
                        ...
                        if (search_string_index == search_string_length){
                            printf("ASCII string found at address : 0x%08x\n\n", string_address);
                    ...
                    if ((address - string_address) % 2 == 0){
                        if (character == search_string[search_string_index]){
                            search_string_index++;
                            if (search_string_index == search_string_length){
                                printf("UNICODE string found at address : 0x%08x\n\n", string_address);
                                start_address = string_address - (string_address % 0x100);
                                search_string_found = true;
                                break;
                            }
	...
	if (total_bytes==0) {
        ...
        my_printf(show_flag, "Peek at 0x%x: %x\n", (unsigned int)start_address, number);
	}
    
    if(dump_flag){
        FILE* fp;
        char filename[100];
        sprintf(filename, "./dump_0x%x_0x%x", start_address, start_address + total_bytes);
        fp = fopen(filename, "w+");
